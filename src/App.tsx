...
import type { CleanAction, PlatformId, ToolId } from './types'
...
/** Tool switcher order + the glyph shown before each localized label. */
const TOOLS: { id: ToolId; glyph: string }[] = [
  { id: 'studio', glyph: '𝔽' },
  { id: 'thread', glyph: '🧵' },
  { id: 'quote', glyph: '🖼️' },
]
...
export default function App() {
  ...
  // Creator Suite tool tabs. Each landing page declares the tool it ranks for
  // (PAGE_DEFAULT_TOOL), so a visitor arriving from search opens straight on
  // that tool; the H1/intro stay in the DOM for every tab so crawlers always
  // see the page's optimised copy.
  const [activeTool, setActiveTool] = useState<ToolId>(seo.defaultTool)
  useEffect(() => {
    setActiveTool(seo.defaultTool)
  }, [seo.defaultTool])
    ...
    <I18nProvider locale={route.locale} page={route.page}>
      <div className="flex min-h-dvh flex-col">
        ...
        <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-6 sm:px-6 sm:py-8">
          ...
          {activeTool === 'thread' && <ThreadSplitter showToast={showToast} />}

          {activeTool === 'quote' && <QuoteCardGenerator showToast={showToast} />}
        </main>
